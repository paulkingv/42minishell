...
/*   Updated: 2026/07/16 16:00:16 by pking            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void child_exe_cmd(int prev_fd, int pipe_fd[2], t_shell *shell)
{
	...
	if (shell->cmd->next) // have to set up where we write to. Pipe FD overwrite STDOUT
		safe_dup2(pipe_fd[1], STDOUT_FILENO);
	exec_close_pipe(pipe_fd);
	if (shell->cmd->redir)
		handle_redirects(shell->cmd->redir);
	envp = env_to_array(shell->env);
	execve(shell->cmd->args[0], shell->cmd->args, envp);
...
void exe_cmdline(t_shell *shell)
{
    ...
    while (shell->cmd)
	{
		...
		{
			exit = exec_builtin(shell->cmd, &shell->env); // idk how to do this w shell struct
		...
		if (pid == 0)
			exit = child_exe_cmd(prev_fd, pipe_fd, shell->cmd, shell->env);
		prev_fd = parent_cleanup_exe_cmd(prev_fd, pipe_fd, shell->cmd);
		shell->cmd = shell->cmd->next;
	}
